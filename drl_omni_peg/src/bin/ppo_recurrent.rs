    ...
    pyo3::Python::with_gil(|py| {
        ...
        match MODE {
            Mode::Train => {
                ...
                let policy = "MlpLstmPolicy".to_object(py);
                ...
                let agent = if let Some(load_model_path) = find_last_checkpoint(&logdir) {
                    ...
                    sb3::contrib::ppo_recurrent::RecurrentPPO::load(
                        py,
                        &load_model_path,
                        env,
                        device.as_ref(py),
                    ...
                    sb3::contrib::ppo_recurrent::RecurrentPPO::new(
                        py,
                        policy.as_ref(py),
                        env,
                        learning_rate.as_ref(py),
                        128,
                        Some((N_ENVS * 8) as i64),
                        8,
                        0.997,
                        0.95,
            ...
            Mode::Eval => {
                ...
                let model = sb3::contrib::ppo_recurrent::RecurrentPPO::load(
                    py,
                    &load_model_path,
                    env,
                    device.as_ref(py),
            ...
            Mode::Enjoy => {
                ...
                let lstm_states = py.None();
                let mut lstm_states = lstm_states.as_ref(py);
                let ones_kwargs = PyDict::new(py);
                ones_kwargs.set_item("dtype", "bool")?;
                let mut episode_starts =
                    py.import("numpy")?
                        .call_method("ones", (N_ENVS,), Some(ones_kwargs))?;

                let mut observation = env.call_method0("reset")?;
                loop {
                    predict_kwargs.set_item("state", lstm_states)?;
                    predict_kwargs.set_item("episode_start", episode_starts)?;
                    let (action, states) = model
                        .call_method("predict", (observation,), Some(predict_kwargs))?
                        .extract::<(&PyAny, &PyAny)>()?;
                    lstm_states = states;

                    let (obs, _rew, done, _info) = env
